...
\begin{titlepage}
	...
	\color{azulUC3M}
	\begin{center}
		\makebox[\textwidth][c]{\includegraphics[width=16cm]{img/Portada_Logo.png}}\\[2.5cm]
		\begin{Large}
			Máster Universitario en Robótica y Automatización\\
			2025--2026\\
			\vspace{2cm}		
			\textsl{Trabajo de Investigación Tutelado}
			\bigskip
			
		\end{Large}
		 	{\Huge Picking Robótico Bimanual Guiado por \\[0.1cm]Tendencias de Moda}\\[0.5cm]
            {\Large \textit{Cierre del bucle IT-OT en intralogística textil mediante modelos visión-lenguaje}}\\
		 	\vspace*{0.5cm}
	 		\rule{10.5cm}{0.1mm}\\
			\vspace*{0.9cm}
			{\LARGE Víctor Martín Parra}\\
			\vspace*{1cm}
		\begin{Large}
			Leganés, 2026\\
		\end{Large}
...
tendencias sociales, empleando un robot colaborativo bimanual, una taxonomía de prendas de 29 tipos genéricos diseñada específicamente para picking industrial \cite{guo2019imaterialist, jia2020fashionpedia} y un
...
visión del robot}. Como se argumenta en la Sección~\ref{sec:relwork}, la
literatura actual trata la manipulación robótica operacional (OT) y el
análisis semántico de tendencias (IT) como silos metodológicos separados;
este trabajo propone salvar esa distancia.
...
la arquitectura propuesta en tres capas, la plataforma robótica y sensorial,
la capa de ejecución robótica y control, y la taxonomía de prendas orientada
a picking industrial, incluyendo el principio de separación de
responsabilidades entre percepción, catálogo y lenguaje. La
Sección~\ref{sec:modes} detalla los cinco modos de operación progresivos
--- percepción y correspondencia de catálogo, reposición geográfica,
inteligencia de tendencias y optimización multiobjetivo --- y presenta los
escenarios de aplicación ilustrativos. La Sección~\ref{sec:metrics} enumera
las métricas de evaluación previstas. La Sección~\ref{sec:plan} resume la
planificación de trabajo. La Sección~\ref{sec:future} discute limitaciones y
trabajo futuro, y la Sección~\ref{sec:conclusion} concluye el artículo.
...
una señal de demanda real --- el llamado \emph{postponement} --- cuenta con
un precedente empresarial ampliamente documentado fuera de la robótica: el
modelo de venta directa y fabricación bajo pedido (\emph{build-to-order})
que popularizó Dell Computer Corporation a partir de los años noventa. Dell
sustituyó la acumulación de inventario especulativo por una
\emph{integración virtual} de su cadena de suministro basada en tecnologías
de la información, de modo que la señal de demanda del cliente final se
propagaba, con una distorsión mínima, hasta el ensamblaje y el
aprovisionamiento de componentes \cite{magretta1998virtual}. Este trabajo
traslada un principio análogo al dominio de la moda: los Modos~3 a~5 de la
arquitectura propuesta (Sección~\ref{sec:modes}) no resuelven qué prenda
concreta prioriza el picking por anticipado, mediante previsión de
catálogo, sino que retrasan esa decisión hasta disponer de una señal de
demanda real --- geográfica en el Modo~3, de tendencia social en el
Modo~4 --- de forma análoga a como la integración virtual de Dell retrasaba
el ensamblaje hasta disponer de la orden del cliente.
...
agente de tendencias (LLM local). Para garantizar tiempos de respuesta compatibles con el ciclo industrial y evitar latencias o problemas de privacidad, la inferencia de Llama-3 \cite{llama3} y CLIP \cite{radford2021clip} se ejecuta en el PC Orquestador (Edge Computing) utilizando formatos cuantizados (GGUF \cite{ggerganov2023llamacpp}, ONNX \cite{onnxruntime}) o motores ligeros (Ollama, TensorRT),
sometido a aprobación por parte de un operario, desde el panel HMI de
planta, antes de tener ningún efecto sobre el Middleware. La capa de
Middleware, construida sobre ROS~2, aloja el
...
\begin{tikzpicture}[
  ...
  gate/.style={rectangle, draw, rounded corners, minimum width=4.6cm,
    minimum height=0.75cm, align=center, thick, fill=black!25, font=\scriptsize},
...
\node[ot] (ot) at (0,-3.1)
  {\textbf{OT --- Robótica}\\[2pt]
   ...
   Controlador de seguridad de planta (PLC)};
\node[gate] (gate) at (0,1.55)
  {Aprobación del operario (panel HMI de planta)};
\draw[arr] (it.south) -- (gate.north);
\draw[arr] (gate.south) -- node[right, xshift=1mm]{tendencia activa} (mw.north);
\draw[arr] (mw.south) -- node[right, xshift=1mm]{coordenadas de picking} (ot.north);
\end{tikzpicture}
\caption{Arquitectura propuesta en tres capas (IT-Middleware-OT). La
aprobación del operario se ejecuta desde el panel HMI de planta (OT) y actúa
como filtro obligatorio antes de que una tendencia propuesta por IT llegue a
activarse en el Middleware.}
\label{fig:arch}
\end{figure}
...
brazos como la asistencia mutua en el agarre de cajas voluminosas --- con
...
posición de agarre de la caja mediante servoing visual. Un autómata
programable (PLC) gestiona la máquina de estados de seguridad de la célula
de trabajo, y un panel de operador (HMI) permite seleccionar el modo de
operación activo, revisar tendencias pendientes de aprobación y supervisar
el estado general del sistema. La Tabla~\ref{tab:hw} resume esta
configuración.
...
Brazo colaborativo (×2) & Picking izquierdo / derecho y asistencia bimanual
  sobre cajas \\
Cámara cenital de profundidad & Detección de cajas y clasificación visual de
  prendas \\
Cámara en muñeca (×2) & Ajuste fino de agarre de la caja mediante servoing
  visual \\
Autómata de seguridad (PLC) & Máquina de estados y supervisión de
  seguridad \\
Panel de operador (HMI) & Selección de modo, aprobación de tendencias,
  estado \\
...
restricción de aceleración cinemática (límites de aceleración/desaceleración) \cite{biagiotti2008trajectory},
...
\textbf{Propuesta} & --- & \textbf{29 tipos} & \textbf{Picking industrial} \\
\bottomrule
\end{tabular}
\end{table}

La taxonomía propuesta organiza sus 29 tipos genéricos en seis familias \cite{guo2019imaterialist, jia2020fashionpedia}
...
Las cajas llegan ya abiertas por los extremos al puesto de percepción --- su
apertura no es una acción que ejecute el sistema, sino una condición previa
del proceso, ver Sección~\ref{sec:robot} ---, de modo que la segunda etapa
observa directamente la prenda en su condición real de almacén: doblada,
bajo iluminación industrial, vista desde una cámara cenital. A partir de esa
imagen, la etapa determina a cuál de los 29 tipos genéricos de la
...
Sección~\ref{sec:modes}-A: la infraestructura de middleware y control es
...
esta propuesta, siguiendo los objetivos de diseño y el plan de trabajo
descritos en las Secciones~\ref{sec:metrics} y~\ref{sec:plan}, constituye el
...
\bibitem{llama3}
AI@Meta, ``The Llama 3 Herd of Models,'' \emph{arXiv preprint
arXiv:2407.21783}, 2024.

\bibitem{ggerganov2023llamacpp}
G.~Gerganov \emph{et~al.}, ``llama.cpp: LLM inference in C/C++,'' GitHub
repository, 2023. [Online]. Available: https://github.com/ggerganov/llama.cpp

\bibitem{onnxruntime}
ONNX Runtime developers, ``ONNX Runtime,'' 2021. [Online]. Available:
https://onnxruntime.ai

\bibitem{biagiotti2008trajectory}
L.~Biagiotti and C.~Melchiorri, \emph{Trajectory Planning for Automatic
Machines and Robots}. Berlin, Germany: Springer, 2008.

\bibitem{magretta1998virtual}
J.~Magretta, ``The Power of Virtual Integration: An Interview with Dell
Computer's Michael Dell,'' \emph{Harvard Business Review}, vol.~76, no.~2,
pp.~72--84, 1998.
