...
\title{Picking Robótico Bimanual Guiado por Tendencias de Moda:\\
Estado del Arte y Propuesta Arquitectónica para el Cierre\\
del Bucle IT-OT en Intralogística Textil}
...
Leganés, España\\
victormpar@gmail.com}
\thanks{Trabajo de Investigación Tutelado (6 ECTS) --- Máster Universitario en
Robótica y Automatización, Universidad Carlos III de Madrid. Curso 2025/2026.
Este documento constituye la fase de revisión del estado del arte y
propuesta arquitectónica previa al desarrollo del Trabajo Fin de Máster.}
}
...
tendencias sociales, empleando un robot colaborativo bimanual, una taxonomía
de prendas de 29 clases diseñada específicamente para picking industrial y un
espacio semántico compartido basado en modelos de lenguaje-visión. El
documento identifica el vacío de investigación que motiva el futuro Trabajo
Fin de Máster, describe la arquitectura propuesta a alto nivel, discute
escenarios de aplicación ilustrativos y presenta la planificación de trabajo
prevista.
\end{abstract}

\begin{IEEEkeywords}
robótica colaborativa, manipulación de textiles, visión por computador,
modelos multimodales, CLIP, picking robótico, integración IT/OT, Industria
4.0, predicción de tendencias de moda, intralogística
\end{IEEEkeywords}
...
4.0. La Figura~\ref{fig:arch} resume esta organización. La capa IT concentra
la inteligencia de negocio: el catálogo clasificado por marketing y el
análisis de tendencias en redes sociales mediante modelos de lenguaje,
...
\node[it] (it) at (0,3.1)
  {\textbf{IT --- Negocio}\\[2pt]
   ...
   Agente de tendencias (redes sociales)\\
...
\subsection{Modos de operación progresivos}
...
\begin{table}[!t]
\caption{Modos de operación propuestos}
\label{tab:modes}
\centering
\footnotesize
...
\begin{table}[!t]
\caption{Plataforma robótica y sensorial propuesta}
\label{tab:hw}
\centering
\footnotesize
...
\section{Taxonomía de Prendas Orientada a Picking Robótico}
\label{sec:taxonomy}
...
\begin{table}[!t]
\caption{Comparación con taxonomías de moda del estado del arte}
\label{tab:taxonomy}
\centering
\footnotesize
...
La taxonomía propuesta organiza sus 29 tipos genéricos en seis familias
...
\section{Percepción y Correspondencia de Catálogo}
\label{sec:vision}

\subsection{Detección y singulación de cajas}
...
\subsection{Clasificación por tipo genérico}
...
cámara cenital --- y determina a cuál de los 29 tipos genéricos de la
...
\subsection{Identificación de la referencia exacta de producto}
...
\section{Inteligencia de Tendencias y Emparejamiento Semántico}
\label{sec:trends}

\subsection{Agente de tendencias}
...
\subsection{Flujo de aprobación humano en el bucle}

Ninguna tendencia detectada por el agente automatizado tiene efecto directo
sobre la operación de picking. Como ilustra la Figura~\ref{fig:approval}, el
...
\subsection{Emparejamiento semántico prenda-tendencia}
...
\section{Reposición Geográfica y Optimización Multiobjetivo}
\label{sec:modes}
...
blando dentro de ese conjunto ya filtrado. De este modo, un mismo pedido para
...
Sección~\ref{sec:relwork}-E. El planificador de picking traduce cada
...
posición real de la prenda observada --- una técnica conocida como servoing
...
coordinación bimanual permite procesar dos prendas en paralelo cuando la
configuración del pedido lo permite, asignando cada prenda a uno de los dos
...
\section{Métricas de Evaluación Previstas}
\label{sec:metrics}

Dado que este trabajo se sitúa en su fase de revisión y propuesta, previa a
la implementación, la Tabla~\ref{tab:metrics} recoge los objetivos de diseño
...
\begin{table*}[!t]
\caption{Objetivos de diseño previstos por componente del sistema}
\label{tab:metrics}
\centering
\footnotesize
...
\midrule
Clasificación visual & mAP@0.5 & Precisión media sobre los 29 tipos de la taxonomía & $> 0.85$ \\
...
Picking robótico & Tiempo de ciclo & Tiempo entre detección y depósito de la prenda & $< 8$ s/prenda \\
...
\section{Casos de Uso Ilustrativos y Aportación Esperada}
\label{sec:usecases}
...
\subsection{Escenario 1: reposición por demanda regional}
...
\subsection{Escenario 2: picking orientado a tendencias virales}
...
\subsection{Escenario 3: gestión integral multiobjetivo}
...
\subsection{Aportación diferencial frente al estado del arte}
...
\begin{table*}[!t]
\caption{Plan de trabajo previsto}
\label{tab:plan}
\centering
\footnotesize
...
En primer lugar, el filtro categórico del emparejamiento semántico depende
...
En segundo lugar, el etiquetado de estilo de cada referencia de catálogo lo
...
En tercer lugar, el equilibrio del catálogo entre familias de estilo es, por
...
J.~Sanchez \emph{et~al.}, ``Robotic manipulation and sensing of deformable
objects in domestic and industrial applications,'' \emph{International
Journal of Robotics Research}, 2018.

\bibitem{lui2023textilesurvey}
K.~Lui \emph{et~al.}, ``A survey on textile manipulation for robotic
systems,'' \emph{IEEE Transactions on Automation Science and Engineering},
2023.
