...
(T4×2 o P100, ~30 h/semana) igual que Colab. Usado de verdad el 2026-09-20 para entrenar
`florence2_base_lora_v2/` (resultado en `memoria/TFM_clasificador_visual_atributos.md` §11.2).
...
`push()` en la URL — hay que copiarlo a la constante `SLUG` antes de poder consultar `estado`.
...
está en el repo), entrena (`entrenar_lora.py`, ~20-25 min en T4/P100) y evalúa en el test set
(`evaluar_modelo.py --sin-zero-shot --sin-1207`). Al final copia a `/kaggle/working/` (lo que
`push_and_run.py bajar` descarga):

- `florence2_base_lora_v2.zip` — el adapter entrenado (con `resultados_entrenamiento.json` y
  `evaluacion_test_v2.txt` ya dentro), para descomprimir en
  `experimentos/vlm_atributos_prenda/modelos/florence2_base_lora_v2/`.
- `resultados_entrenamiento.json` y `evaluacion_test_v2.txt` sueltos también, por comodidad.
